...
file_to_search.bind('<Button-1>', lambda event, file_to_search = file_to_search : search.makeEntryEmpty(file_to_search))

# Calling the start_search method after clicking on Search File Button
search_button.bind('<Button-1>', lambda event, file_to_search = file_to_search, window = window : search.start_search(file_to_search, window))
